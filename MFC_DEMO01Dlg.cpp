...
//	ON_BN_CLICKED(IDC_BUTTON1, &CMFC_DEMO01Dlg::OnBnClickedButton1)
	...
	ON_STN_CLICKED(IDC_PIC_STATIC, &CMFC_DEMO01Dlg::OnStnClickedPicStatic)
	ON_EN_CHANGE(IDC_OUT_EDIT1, &CMFC_DEMO01Dlg::OnEnChangeOutEdit1)
	//ON_BN_CLICKED(IDC_START_BUTTON2, &CMFC_DEMO01Dlg::OnBnClickedStartButton2)
	ON_BN_CLICKED(IDC_START_BUTTON1, &CMFC_DEMO01Dlg::OnBnClickedStartButton1)
	ON_BN_CLICKED(IDC_START_BUTTON2, &CMFC_DEMO01Dlg::OnBnClickedStartButton2)
	ON_BN_CLICKED(IDC_START_BUTTON3, &CMFC_DEMO01Dlg::OnBnClickedStartButton3)
END_MESSAGE_MAP()
...
BOOL CMFC_DEMO01Dlg::OnInitDialog()
{
	...
	m_comboPic.AddString(_T("bound"));  //骨骼
	m_comboPic.AddString(_T("level"));  //深度图
	m_comboPic.AddString(_T("RGB"));
	m_comboPic.AddString(_T("IR"));
	//也可以使用下面这种,可以调节序号
...
void CMFC_DEMO01Dlg::OnBnClickedStartButton1()     
{
	...
	GetDlgItem(IDC_OUT_EDIT1)->SetWindowText(_T(""));
	if (SUCCEEDED(hr))
	{	
			myKinect.Update(1);								//刷新骨骼和深度数据	
...
//身高 距离设备2-2.5米
void CMFC_DEMO01Dlg::OnBnClickedStartButton2()           
{
	GetDlgItem(IDC_OUT_EDIT1)->SetWindowText(_T(""));
	// TODO: 在此添加控件通知处理程序代码
	HRESULT hr = myKinect.InitializeDefaultSensor();		//初始化默认Kinect
...
//立定跳远
void CMFC_DEMO01Dlg::OnBnClickedStartButton3()
{
	GetDlgItem(IDC_OUT_EDIT1)->SetWindowText(_T(""));
	HRESULT hr = myKinect.InitializeDefaultSensor();		//初始化默认Kinect
	if (SUCCEEDED(hr))
	{
		myKinect.Update(3);								//刷新骨骼和深度数据	
...
void CMFC_DEMO01Dlg::OnStnClickedPicStatic()
{
	// TODO: 在此添加控件通知处理程序代码
}


void CMFC_DEMO01Dlg::OnEnChangeOutEdit1()
{
	// TODO:  如果该控件是 RICHEDIT 控件，它将不
	// 发送此通知，除非重写 CDialogEx::OnInitDialog()
	// 函数并调用 CRichEditCtrl().SetEventMask()，
...
void CMFC_DEMO01Dlg::OnBnClickedButton2()
{
	// TODO: 在此添加控件通知处理程序代码
}
